...
def custom_insert(input_list, index, value):
    ...
    """
    mylen = custom_len(input_list)
    if index == 0:
        input_list[:0] = [value]
    elif index == mylen:
        input_list[mylen:] = [value]
    else:
        input_list[mylen:] = [0]
        input_list[index+1:mylen+1] = input_list[index:mylen]
...
def custom_reverse(input_list):
    ...
    for i in range(custom_len(input_list)/2):
        temp = input_list[i]
        input_list[i] = input_list[from_end]
        input_list[from_end] = temp
        from_end -= 1
